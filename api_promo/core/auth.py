...
async def autenticar_usuario_estabelecimento(email: EmailStr, senha: str, db: AsyncSession) -> Optional[UsuarioEstabelecimentoModel]:
    async with db as session:
        query = select(UsuarioEstabelecimentoModel).filter(UsuarioEstabelecimentoModel.email == email)
        result = await session.execute(query)
        ...
        if not verificar_senha(senha, usuario_estabelecimento.senha):
            return None
